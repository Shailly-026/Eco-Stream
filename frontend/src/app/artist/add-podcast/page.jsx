...
//       method: "POST",
//       headers: { "Content-Type": "application/json", 'x-auth-token': localStorage.getItem('artist') },
...
//             <option value="Technology">Spiritual</option>
//             <option value="Health">Motivational</option>
//             <option value="Comedy">Comedy</option>
//             <option value="Education">Adventures</option>
//             <option value="Education">Education</option>
//             <option value="News">horror</option>
//           </select>
...
export default function AddPodcast() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/podcast/add`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          'x-auth-token': localStorage.getItem('artist')
        },
        body: JSON.stringify(podcastData),
      });
  ...
  const handleFileUpload = (e) => {
    ...
    axios.post('https://api.cloudinary.com/v1_1/dssjgtsjl/auto/upload', formData)
      .then((result) => {
        toast.success('Audio Uploaded Successfully');
        setAudioFile(result.data.url);
      }).catch((err) => {
  ...
  const handleImageUpload = (e) => {
    ...
    axios.post("https://api.cloudinary.com/v1_1/dssjgtsjl/image/upload", formDataImg)
      .then((result) => {
        toast.success("Image Uploaded Successfully");
        setFormData(prev => ({ ...prev, coverImageUrl: result.data.url }));
      })
    ...
    <div className="min-h-screen w-full bg-gray-900 text-white flex justify-center items-center">
      ...
      <div className="max-w-lg w-full bg-gray-800 p-6 rounded-lg shadow-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            className="w-full py-2 bg-purple-500 rounded-lg hover:bg-purple-600 disabled:opacity-50"
            disabled={loading}
          >
            {loading ? "Adding..." : "Add Podcast"}
          </button>
