...
const UserProfile = () => {
  ...
  return (
    <div className="min-h-screen w-full bg-black text-gray-100 font-sans pb-24">
      ...
      {/* Footer */}
      <footer className="fixed bottom-0 left-0 right-0 bg-black border-t border-gray-800 p-3 flex justify-around backdrop-blur-sm bg-opacity-90 z-30">
        {[ 
          { icon: Headphones, label: "Discover" },
          { icon: BookmarkPlus, label: "Library" },
          { icon: UserCircle, label: "Profile" }
        ].map(({ icon: Icon, label }, i) => (
          <button
            key={i}
            className={`flex flex-col items-center transition ${label === "Discover" ? "text-purple-400" : "text-gray-500 hover:text-purple-300"}`}
          >
            <Icon size={20} />
